...
template <class F> using Polygon = vector<Point<F>>;  // Representação de um poligono
...
template <class F>
double area(const Polygon<F>& poly) {
...
int main() {                                                                                           
	...
	while (cin >> N) {
		...
		Polygon<int> S;  // Vetor que contém todos os pontos que estão em um dado espaço S
		...
		Polygon<int> convex = convexHull(S);  // Vetor que contém a envoltória convexa

		double tamanho = area(convex);
		cout << tamanho << endl;
	}  		
